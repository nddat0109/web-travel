...
class ContractController extends Controller
{
    public function new_contract(){
        if(Session::get('position_id')==4 || Session::get('position_id')==1){
            ...
            toastr()->warning('Bạn không có quyền này!!');
    ...
    public function save_contract(Request $request){
       ...
       // $contract->customer_id=Session::get('customer_id');
        ...
        $data=array('title'=>'Thư phản hồi','body1'=>'Cám ơn bạn đã lựa chọn chúng tôi!',
        ...
        Mail::send('Page_Views.mail_exchange',$data,function ($message) use ($from_host,$to_email){
            $message->to($to_email)->subject('Thư phản hồi!');
            $message->from($to_email,$from_host);
            $message->attach('Admins/contract_pdf/hop-dong-du-lich94.pdf');
        });
    ...
    public function update(Request $request){
        ...
        ->join('tbl_booking','tbl_contract.booking_id','=','tbl_booking.booking_id')->first();
         if($contract->contract_status==3){
            toastr()->success('Hợp đồng đã được hoàn thành!', 'Congrats', ['timeOut' => 2000]);
            return Redirect::back();

        }
        else if($contract->contract_status==2){
            if($request['contract_status']==1  or $request['contract_status']==0 ){
                toastr()->success('Hợp đồng đang tiến hành...', 'Congrats', ['timeOut' => 2000]);
                return Redirect::back();
            }
        }
         else if($contract->contract_status==0){
            toastr()->success('Hủy hợp đồng thành công', 'Congrats', ['timeOut' => 2000]);
            return Redirect::back();
        }
