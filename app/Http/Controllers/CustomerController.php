...
class CustomerController extends Controller
{
    ...
    public function login_customer(Request $request){
        ...
        if($result){
            ...
            toastr()->success('Đăng nhập thành công', 'Congrats', ['timeOut' => 2000]);
            return Redirect::to('/');
        }
        else {
            toastr()->error('Tài khoản hoặc mật khẩu không đúng!', 'Oops!');
            return back();
        }
    }
    ...
    public function update_profile(Request $request,$customer_id){
        ...
        $customer->save();
        toastr()->success('Cập nhật dữ liệu thành công!', 'Congrats', ['timeOut' => 2000]);
        Session::put('customer_name',$customer->customer_name);
        Session::put('customer_email',$customer->customer_email);
        Session::put('customer_id',$customer->customer_id);
        Session::put('customer_phone_number',$customer->customer_phone_number);
        Session::put('customer_address',$customer->customer_address);
        Session::put('customer_nationality',$customer->customer_nationality);
        return redirect()->back();
    }  
    ...
    public function add_customer(Request $request){
     $request->validate([
        ...
        'customer_phone_number' => ['required','numeric','unique:tbl_customer'],
        'customer_password' => ['required', 'string', 'min:6'],
            ...
            'customer_email.email' => 'Email không đúng định dạng',
            'customer_email.max' => 'Email không quá 255 ký tự',
            'customer_phone_number.numeric' => 'Vui lòng điền đúng SĐT',
            'customer_email.unique' => 'Email đã tồn tại',
            'customer_phone_number.unique' => 'Số điện thoại đã tồn tại',
            'customer_password.required' => 'Mật khẩu là trường bắt buộc',
        ...
        if($res){
            ...
            Session::put('customer_nationality',$customer->customer_nationality);
            $_SESSION['status_login'] = true;
            toastr()->success('Đăng ký thành công', 'Congrats', ['timeOut' => 2000]);
            return Redirect::to('/');
        }else{
    ...
    public function changePassword(Request $request){
        ...
        if(md5($request->customer_password) == $customer->customer_password){
            $customer->customer_password = md5($request->new_cus_password);
            $customer->save();
            return Redirect::to('/login-index');
        }
        toastr()->warning('Đã nhập sai!', 'Opps', ['timeOut' => 2000]);
        return Redirect::back();
    }

    public function changePasswordCustomer(Request $request){
        return view('Page_Views.Account.ChangePassword');
    }
