...
class HomeController extends Controller
{
    public function index(){
        $dest=Destination::where('destination_status',1)->inRandomOrder()->get();
        $typetour=Typetour::where('typetour_status',1)->orderby('typetour_id','asc')->get();
    ...
    public function dashboard(){
        ...
        if(Session::get('position_id')==1){
            ...
            for($i=0;$i<=11;$i++){
                $month_numb=$i+1;
                $revenue_per_month= Contract::whereyear('contract_date','=',Session::get('year_revenue'))->whereMonth('contract_date','=',$i+1)->sum('contract_total_price')/1000000;
                array_push($month_revenue,$revenue_per_month);
            }
            $highest_month= max($month_revenue)*1000000;
            ...
            $total_revenue=$first_part + $second_part + $third_part + $fourth_part;
